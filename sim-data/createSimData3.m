function data_struct = createSimData3(num_neurons, rng_seed)
...
rng(rng_seed);
T = 5000;                       % total experiment duration (sec)
% bin_size = 0.050;               % sec
num_bins = T/bin_size;
...
if dataset == 1
    % stim signal constants
    stim_nl = 'quad';           % 'lin' | 'quad' | 'rect_quad' | 'relu' | 'sigmoid'
...
switch gain_nl
    ...
    case 'exp'
        ...
        gain_coupling = rand(num_neurons,1);
        % normalize
        gain_sig_filt = 1.0 * gain_sig_filt/std(gain_sig_filt);  % 2 is new for gam revisions
        % pass through nl
